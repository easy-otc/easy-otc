...
public class MailSender {
    ...
    static {
        // 用于连接邮件服务器的参数配置
        PROPS.setProperty("mail.smtp.port", "587"); // 使用的协议（JavaMail规范要求）
        PROPS.setProperty("mail.smtp.host", "smtp.qq.com"); // 发件人的邮箱的 SMTP 服务器地址
        PROPS.setProperty("mail.user", SENDER_EMAIL); // 需要请求认证
    ...
    public static boolean sendMail(String receiveMail, String subject, String content) {
        try {
            ...
            // 3. 创建一封邮件
            MimeMessage message = createMimeMessage(session, receiveMail, subject, content);
            ...
            transport.connect(SENDER_EMAIL, SENDER_AUTH_CODE);
     ...
     * @throws Exception
     */
    private static MimeMessage createMimeMessage(Session session, String receiveMail, String subject, String content) throws Exception {
        // 1. 创建一封邮件
        ...
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, RECIVER_NICK, "UTF-8"));
